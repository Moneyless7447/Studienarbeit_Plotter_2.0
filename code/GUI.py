...
class GUI:
    def __init__(self):
        ...
        self.window.configure(bg="#36373d")
        ...
        self.apply_button = tk.Button(self.window, text='Apply', command=self.apply_changes, bg='#76798a', fg='white', width=28)
        self.view = FigureCanvasTkAgg(self.fig, self.window)
        self.bool_title = True
        # self.bool_title.set(0)
        self.check_title = tk.Checkbutton(self.window, text='Titles', command=self.toggle_title, onvalue=1, offvalue=0, background='#36373d', foreground='white')
        self.check_title.select()
        self.bool_name = False
        # self.bool_name.set(0)
        self.check_name = tk.Checkbutton(self.window, text='Name', command=self.toggle_name, background='#36373d', foreground='white')
        self.check_name.deselect()
        self.bool_symbols = True
        # self.bool_symbols.set(0)
        self.check_symbols = tk.Checkbutton(self.window, text='3D-Symbols', command=self.toggle_symbols, background='#36373d', foreground='white')
